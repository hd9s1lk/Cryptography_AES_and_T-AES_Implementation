...
int main(int argc, char *argv[]) { 
    ...
    if(argc == 5) {
        ...
        operation_success = process_t_aes_ni_decryption(base_round_keys, tweak_key, key_length);
    }else if (argc == 4) {
        ...
        operation_success = process_t_aes_decryption(base_round_keys, tweak_key, key_length);
        ...
        key_expansion(main_key, base_round_keys, s_box, key_length);
        operation_success = process_aes_decryption(base_round_keys, key_length);
    }

    if(operation_success == -1) {
        fprintf(stderr, "Debug: Binary file to small to decrypt\n");        
    }
